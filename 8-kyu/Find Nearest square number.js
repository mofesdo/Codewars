...
function nearestSq(n){
    //Check if number is already a perfect square, if not then round the sqrt and square it to find nearest perfect square.
    return Number.isInteger(Math.sqrt(n)) ? n :  Math.round(Math.sqrt(n)) *  Math.round(Math.sqrt(n));
  }